...
import { perspectivasService } from './perspectivasService';
...
export const generateDashboardPDF = async () => {
  try {
    ...
    // Sección de distribución por objetivos
    pdf.setFontSize(16);
    pdf.text('Distribución por Objetivos', 20, yPosition);
    yPosition += 15;
    
    // Agrupar KPIs por objetivo
    const kpisPorObjetivo: Record<string, KPIData[]> = {};
    kpis.forEach((kpi: KPIData) => {
      const nombreObjetivo = `Objetivo ${kpi.objetivo_id}`;
      
      if (!kpisPorObjetivo[nombreObjetivo]) {
        kpisPorObjetivo[nombreObjetivo] = [];
      }
      kpisPorObjetivo[nombreObjetivo].push(kpi);
    });
    
    // Mostrar distribución
    pdf.setFontSize(12);
    Object.entries(kpisPorObjetivo).forEach(([objetivo, kpisArray]) => {
      pdf.text(`${objetivo}: ${kpisArray.length} KPIs`, 25, yPosition);
      yPosition += 8;
    });
    ...
    pdf.setFontSize(10);
    kpis.slice(0, 15).forEach((kpi: KPIData, index: number) => {
